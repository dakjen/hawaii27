...
export async function sendPushToAll(payload, excludeSubscriber = null) {
  let wp;
  try {
  ...
  let subs;
  try {
    subs = excludeSubscriber
      ? await sql`SELECT id, endpoint, p256dh, auth FROM push_subscriptions
                  WHERE subscriber IS DISTINCT FROM ${excludeSubscriber}`
      : await sql`SELECT id, endpoint, p256dh, auth FROM push_subscriptions`;
  } catch (_) {
...
// Push to a specific set of subscribers (by traveler id / 'admin').
export async function sendPushTo(subscribers, payload) {
  let wp;
  try { wp = await ensureConfigured(); } catch (_) { return { sent: 0, error: 'web-push-unavailable' }; }
  if (!wp) return { sent: 0, skipped: 'vapid-not-configured' };
  ...
  let subs;
  try {
    subs = await sql`SELECT id, endpoint, p256dh, auth FROM push_subscriptions WHERE subscriber = ANY(${subscribers})`;
  } catch (_) { return { sent: 0, skipped: 'no-subscriptions-table' }; }
  ...
  if (stale.length) { try { await sql`DELETE FROM push_subscriptions WHERE id = ANY(${stale})`; } catch (_) {} }
  return { sent, removed: stale.length };
}
